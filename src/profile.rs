...
fn to_profile(p: impl Provider) -> Result<Profile, super::Error> {
    ...
    let truc = serde_json::from_value::<Profile>(data.get(&profile_name).unwrap().clone()).unwrap();
...
mod tests {
    ...
    fn test_profile_deserialize_full() {
        // Profile's custom deserializer uses `rename_all = "lowercase"` on its
        // field-identifier enum, which folds every field name to plain lowercase
        // with no separators: access_key → "accesskey", tls_skip_verify →
        // "tlsskipverify", x509clientcert, etc.
        let json = serde_json::json!({
            ...
            "x509clientcert": "/path/cert.pem",
            "x509clientkey": "/path/key.pem",
            "x509clientcertb64": "Y2VydA==",
            "x509clientkeyb64": "a2V5"
        });
    ...
    fn test_profile_tls_skip_verify_false() {
        // rename_all = "lowercase" strips underscores: tls_skip_verify → "tlsskipverify"
        let json = serde_json::json!({ "tlsskipverify": false });
        let p: Profile = serde_json::from_value(json).expect("deserialize");
        assert!(!p.tls_skip_verify);
    }
